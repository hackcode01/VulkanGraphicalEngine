...
b8 gameUpdate(Game* gameInstance, f32 deltaTime) {
    ...
    if (!vec3_compare(z, velocity, 0.0002f)) {
        ...
        state->cameraPosition.x += velocity.x * temp_move_speed * deltaTime;
        state->cameraPosition.y += velocity.y * temp_move_speed * deltaTime;
        state->cameraPosition.z += velocity.z * temp_move_speed * deltaTime;
        state->cameraViewDirty = true;
    }
